...
class ReceiveSharingIntent: NSObject {
    ...
    @objc
    func getCheckboxStates(_ resolve: @escaping RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock) {
        let appDomain = Bundle.main.bundleIdentifier!
        let userDefaults = UserDefaults(suiteName: "group.\(appDomain)")
                if let checkboxStates = userDefaults?.dictionary(forKey: "checkboxStates") as? [String: Bool] {
            resolve(checkboxStates)
//                    clearCheckboxStates()

        } else {
            let error = NSError(domain: "", code: 400, userInfo: nil)
            reject("no_checkbox_states", "No checkbox states found", error)
        }
    }
    
    @objc
    func clearCheckboxStates() {
        // Get the app's bundle identifier
        let appDomain = Bundle.main.bundleIdentifier!
        
        // Access the shared UserDefaults using the app's group identifier
        let userDefaults = UserDefaults(suiteName: "group.\(appDomain)")
        
        // Remove the stored checkbox states
        userDefaults?.removeObject(forKey: "checkboxStates")
        
        // Synchronize the UserDefaults to ensure the changes are saved immediately
        userDefaults?.synchronize()
        
        // Print a log message indicating that the checkbox states have been cleared
        print("Checkbox states cleared")
    }
